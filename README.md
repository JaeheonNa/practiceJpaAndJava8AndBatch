...
13. [고민 및 접근 해결 방식](#13-고민-및-접근-해결-방식)  
...
## 0. 개요
    ...
    테스트는 소스 코드를 다운 받아 IDE에서 실행시킨 후 API 명세에 따라 실행하시면 됩니다. 
    8080포트를 사용했습니다. DB는 h2 인메모리 버전으로 설정돼 있습니다. (제시된 상품은 1.[보험 및 담보 초기 데이터 생성 API]를 실행하셔야 DB에 저장됩니다.)
    Postman 사용을 권장드립니다.
...
```
    ...
    6. 계약의 수정과 관련하여, 계약 상태 수정, 계약 기간 수정, 담보 추가, 담보 삭제로 API를 나눌 것인지, 혹은 하나로 합쳐서 하나의 요청으로 전부 처리할 것인지 고민했습니다.
